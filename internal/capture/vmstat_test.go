...
func TestVMStat_CaptureToFile(t *testing.T) {
	...
	t.Run("creates output file in correct location", func(t *testing.T) {
		...
		if err == nil {
			...
			// If vmstat command fails (not available), that's acceptable
			// The test verifies the error handling path works
			t.Logf("vmstat capture failed (expected in environments without vmstat): %v", err)
		}
	})
}
