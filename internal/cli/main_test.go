...
func TestProcessLogFile(t *testing.T) {
	...
	test := func(t *testing.T, dir string, fname string, out string) {
		...
		gc, err := capture.ProcessGCLogFile(filepath.Join(dir, fname), filepath.Join(dir, out), "", 0)
		if err != nil {
			t.Fatal(err)
		}
		gc.Seek(0, 0)
	...
	t.Run("gcPath-not-exists", func(t *testing.T) {
		_, err := capture.ProcessGCLogFile("gc-rotation-logs/0-current/1/gc.log.current", "gc-rotation-logs/0-current/1/gc.log", "", 0)
...
func TestCaptureCmd(t *testing.T) {
	_, err := utils.RunCaptureCmd(123, "echo $pid")
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestServer(t *testing.T) {
	...
	defer noGC.KillAndWait()
	noGC2, err := utils.CommandStartInBackground(utils.Command{"java", "-cp", "../capture/testdata/", "MyClass"})
	if err != nil {
		t.Fatal(err)
	}
	defer noGC2.KillAndWait()

	s, err := utils.NewServer("localhost", 0)
	if err != nil {
		t.Fatal(err)
	}
